...
test('dashboard navigation, matrix, latest expansion, status, preserved notes',async()=>{
  ...
  q('#advancementBtn').click();assert.equal(w.document.querySelectorAll('#seasonQualification .team-origin').length,80);assert.doesNotMatch(q('#seasonQualification').textContent,/来源待确认|路径未分类/);q('[data-tab="statistics"]').click();assert.equal(q('#seasonStatistics').hidden,false);assert.equal(q('#seasonQualification').hidden,true);assert.equal(w.document.querySelectorAll('.season-cell').length,288);
