(function(){
  ...
  page.innerHTML=`
    <section class="qualification-hero">
      <div><p class="eyebrow">QUALIFYING TRACKER · 2026/27</p><h1>资格赛<span>战绩中心</span></h1><p>从第一轮到附加赛，集中查看累计表现、晋级路径、当前进度与未来赛程。</p><div class="qualification-stamp"><i></i>数据核对至 2026-08-03 · 北京时间</div></div>
      <div class="qualification-scoreboard"><article><b>56</b><span>已完成比赛</span></article><article><b>20</b><span>第三轮球队</span></article><article><b>7</b><span>联赛阶段席位</span></article><article><b>2 / 4</b><span>已完成轮次</span></article></div>
    </section>
    <section class="qualification-progress" aria-label="资格赛进度">
      <div class="progress-head"><div><p class="eyebrow">ROUND PROGRESS</p><h2>赛程进度</h2></div><strong>第三轮即将开赛</strong></div>
      <div class="round-track">
        ...
        <i></i><article class="current"><span>03</span><div><b>第三轮</b><small>8月4/5、11日</small></div><em>0 / 20场</em></article>
        <i></i><article><span>04</span><div><b>附加赛</b><small>8月18/19、25/26日</small></div><em>0 / 14场</em></article>
      </div>
    </section>
    ...
    <section class="qualification-schedule">
      <div class="section-title"><div><p class="eyebrow">UPCOMING FIXTURES</p><h2>第三轮赛事安排</h2></div><span>全部为北京时间 · 共20场</span></div>
  ...
  function renderFixtures(path='all'){
    ...
    fixtures.innerHTML=days.map(date=>`<article class="fixture-day"><header><time>${date}</time><span>${new Intl.DateTimeFormat('zh-CN',{weekday:'long'}).format(new Date(date+'T12:00:00+08:00'))}</span></header>${list.filter(match=>match[0]===date).map(([,time,round,route,home,away])=>`<div class="qual-match"><time>${time}</time><span class="path-tag ${route==='联赛路径'?'league':''}">${route}</span><strong>${name(home)}<small>${home}</small></strong><i>VS</i><strong>${name(away)}<small>${away}</small></strong><em>${round.replace('第三轮·','')}</em></div>`).join('')}</article>`).join('');
  }
  renderFixtures();
