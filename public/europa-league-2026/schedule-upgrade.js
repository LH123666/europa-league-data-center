(function(){
  ...
  function club(raw){const t=window.uelTeamInfo(raw),history=context[t?.name||raw];return `<span class="schedule-club"><strong>${esc(t?.zh||raw)}</strong><small>${esc(t?.name||raw)}</small><span class="schedule-context"><em class="pot-${t?.pot||0}">${t?.pot?`第${t.pot}档`:'档位待核实'}</em><em>${esc(window.uelEntryLabels[t?.entry]||'参赛来源待核实')}</em><em class="domestic" title="2025/26国内成绩或杯赛资格依据">${esc(history?.[0]||'国内资格依据待核实')}</em><em class="europe ${history?.[1]==='上季无欧战记录'?'no-record':history?.[1]==='上季未晋级欧战'?'not-qualified':''}" title="2025/26欧洲俱乐部赛事表现">${esc(history?.[1]||'上季欧战表现待核实')}</em></span></span>`;}
  function render(fixtures,predictionSummary,open){
    ...
    if(!document.querySelector('#scheduleMetadataNote'))grid.insertAdjacentHTML('beforebegin','<p id="scheduleMetadataNote">北京时间已按夏令时/冬令时转换，日期同步跨日。国内信息显示2025/26联赛名次、冠军或杯赛资格依据；“无欧战记录”和“未晋级欧战”按UEFA原始口径区分。<a href="https://www.uefa.com/uefaeuropaleague/news/02a8-217364fd42e6-7e49801d8daf-1000--2026-27-europa-league-meet-the-league-phase-teams/" target="_blank" rel="noopener">UEFA 36队官方档案 ↗</a></p>');
  }
