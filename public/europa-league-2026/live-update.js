(function(){
  document.head.insertAdjacentHTML('beforeend','<link rel="stylesheet" href="live-update.css"><link rel="stylesheet" href="fixture-detail.css"><link rel="stylesheet" href="fixture-table.css"><link rel="stylesheet" href="fixture-readable.css"><link rel="stylesheet" href="fixture-results.css"><link rel="stylesheet" href="fixture-stats.css"><link rel="stylesheet" href="fixture-stats-readable.css"><link rel="stylesheet" href="modal-standings.css"><link rel="stylesheet" href="modal-standings-large.css"><link rel="stylesheet" href="fixture-score-status.css"><link rel="stylesheet" href="fixture-home-icon.css"><link rel="stylesheet" href="fixture-alignment.css"><link rel="stylesheet" href="fixture-goal-splits.css"><link rel="stylesheet" href="prediction-editor.css"><link rel="stylesheet" href="prediction-summary-readable.css"><link rel="stylesheet" href="team-notes.css"><link rel="stylesheet" href="uel-theme.css">');
  const nav=document.querySelector('nav'),main=document.querySelector('main'),seasonElement=document.querySelector('.season');
  ...
  seasonElement.insertAdjacentHTML('beforebegin',`<div class="update-wrap"><span class="updated-at" id="updatedAt">${window.uelSeason.current?'官方赛程 · 08-29':'完整静态归档'}</span><button class="update-btn" id="updateBtn"><span class="refresh-icon">↻</span><span class="label">${window.uelSeason.current?'更新数据':'重新载入归档'}</span></button></div>`);
  main.insertAdjacentHTML('beforeend',`<section class="schedule-section" id="schedulePage"><div class="section-head"><div><p class="eyebrow">${window.uelSeason.current?'UPCOMING FIXTURES':'COMPLETE FIXTURE ARCHIVE'}</p><h2>${window.uelSeason.current?'未来赛程':'完整赛程'}</h2></div><span id="fixtureCount">正在载入…</span></div><div class="schedule-grid" id="scheduleGrid"><div class="empty-schedule">正在载入${window.uelSeason.current?'最新':'历史'}赛程…</div></div><div class="data-source-panel"><b>${window.uelSeason.current?'未来赛程':'历史赛程'}数据来源</b><span>${window.uelSeason.current?'UEFA 欧罗巴联赛官方比赛接口（赛果与未来赛程）':'UEFA 官方比赛接口生成的完整静态赛季归档'}</span><a href="https://www.uefa.com/uefaeuropaleague/fixtures-results/" target="_blank" rel="noopener">UEFA 赛程与赛果</a><a href="https://match.uefa.com/v5/matches?competitionId=14&amp;seasonYear=${window.uelSeason.key==='2024-25'?2025:window.uelSeason.key==='2025-26'?2026:2027}&amp;order=ASC&amp;offset=0&amp;limit=400" target="_blank" rel="noopener">UEFA 官方结构化比赛数据</a></div></section>`);
  ...
  const predictionStorageKey=window.uelSeason.current?'uel36-match-predictions-v1':`uel36-match-predictions-v1:${window.uelSeason.key}`;
  ...
  const teamNoteStorageKey=window.uelSeason.current?'uel36-team-notes-v1':`uel36-team-notes-v1:${window.uelSeason.key}`;
  const loadTeamNotes=()=>{try{return JSON.parse(localStorage.getItem(teamNoteStorageKey)||'{}')}catch(e){return {}}};
  const teamNotes=loadTeamNotes();
  ...
  const recalc=()=>{
    ...
    teams.forEach(t=>{const s=stat.get(t[1]),official=officialStandings.get(t[1]);t[3]=official?.p??s.p;t[4]=official?.w??s.w;t[5]=official?.d??s.d;t[6]=official?.l??s.l;t[7]=official?.gf??s.gf;t[8]=official?.ga??s.ga;t[9]=official?.pts??(s.w*3+s.d);t[10]=s.form.slice(-5).join('')});
    teams.sort((a,b)=>officialStandings.size?officialStandings.get(a[1]).rank-officialStandings.get(b[1]).rank:b[9]-a[9]||(b[7]-b[8])-(a[7]-a[8])||b[7]-a[7]);render(document.querySelector('#search').value);renderResults();
    document.querySelector('.hero-stat b').textContent=matches.length;
  };
  const renderSchedule=()=>window.uelSchedule.render(upcoming,predictionSummary,openFixtureDetail);
  const openTeamLive=name=>{
    const t=teams.find(x=>x[1]===name),games=matches.filter(m=>m[1]===name||m[2]===name),rank=teams.indexOf(t)+1;
    document.querySelector('#drawerContent').innerHTML=`<div class="team-hero"><div class="bigbadge">${t[2]}</div><p class="eyebrow">${window.uelSeason.label} SEASON · ${window.uelSeason.current?'当前':'最终'}第 ${rank} 名</p><div class="team-name-note"><div class="team-name-block"><h2>${t[0]}<small>（${t[1]}）</small></h2><span>UEFA Europa League · Europe</span></div><div class="team-note-editor"><div class="team-note-head"><label for="teamNoteInput">球队备注</label><button type="button" id="saveTeamNote">保存</button></div><textarea id="teamNoteInput" maxlength="300" placeholder="添加对该球队的观察、阵容变化或比赛判断…">${escapeHtml(teamNotes[name]||'')}</textarea><small><span id="teamNoteStatus">保存在当前设备 · Ctrl+Enter保存</span><span id="teamNoteCount">${(teamNotes[name]||'').length}/300</span></small></div></div></div><div class="summary"><div><b>${t[9]}</b><span>积分</span></div><div><b>${t[4]}</b><span>胜</span></div><div><b>${t[5]}</b><span>平</span></div><div><b>${t[6]}</b><span>负</span></div></div><div class="history"><div class="history-title"><h3>本赛季全部比赛</h3><span>共 ${games.length} 场</span></div><div class="history-label"><span>日期</span><span>对手（联赛阶段排名）</span><span>半场</span><span>全场</span></div>${games.map(m=>{let home=m[1]===name,sc=m[3].split('-').map(Number),a=home?sc[0]:sc[1],b=home?sc[1]:sc[0],o=a>b?'W':a===b?'D':'L',opp=home?m[2]:m[1],ot=teams.find(x=>x[1]===opp),orank=ot?teams.indexOf(ot)+1:'–';return `<div class="game"><span>${m[0].slice(5)}</span><span class="opponent"><i class="outcome ${o.toLowerCase()}">${o==='W'?'胜':o==='D'?'平':'负'}</i><i class="rank-chip" style="${ot?`background:${rankColor(orank)}`:'background:#7c8b86'}" title="联赛阶段第 ${orank} 名">${orank}</i><span class="opponent-name">${home?'主':'客'} · ${display(opp)}</span></span><span class="ht">${m[4]}</span><span class="score">${m[3]}</span></div>`}).join('')}</div><div class="data-source-panel"><b>球队比赛数据来源</b><span>UEFA官方比赛归档；无法可靠还原的半场比分显示“—”</span><a href="https://www.uefa.com/uefaeuropaleague/history/" target="_blank" rel="noopener">UEFA ${window.uelSeason.label}赛季资料</a></div>`;
    const noteInput=document.querySelector('#teamNoteInput'),noteStatus=document.querySelector('#teamNoteStatus'),noteCount=document.querySelector('#teamNoteCount');
    document.querySelector('#saveTeamNote').onclick=()=>{const value=noteInput.value.trim();if(value)teamNotes[name]=value;else delete teamNotes[name];localStorage.setItem(teamNoteStorageKey,JSON.stringify(teamNotes));noteStatus.textContent=value?'已保存':'备注已清空';noteStatus.classList.add('saved');toast(value?`已保存${t[0]}的备注`:`已清空${t[0]}的备注`);setTimeout(()=>{noteStatus.textContent='保存在当前设备';noteStatus.classList.remove('saved')},1800)};
    noteInput.oninput=()=>noteCount.textContent=`${noteInput.value.length}/300`;
    noteInput.onkeydown=e=>{if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)){e.preventDefault();document.querySelector('#saveTeamNote').click()}};
  ...
  const tableHead=(future=false)=>`<div class="detail-table-head ${future?'future-head':''}"><span>日期</span><span>比赛</span>${future?`<span>${window.uelSeason.current?'时间':'比分'}</span>`:'<span>半场</span><span>全场</span>'}</div>`;
  ...
  async function fetchH2H(home,away){
    if(!window.uelSeason.current)return matches.filter(m=>(m[1]===home&&m[2]===away)||(m[1]===away&&m[2]===home)).sort((a,b)=>b[0].localeCompare(a[0])).slice(0,12);
    const years=[2023,2024,2025];let found=[];
    const payloads=await Promise.all(years.map(y=>fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.europa/scoreboard?dates=${y}&limit=500`).then(r=>r.ok?r.json():{events:[]}).catch(()=>({events:[]}))));
  ...
  async function openFixtureDetail(home,away,date='',time=''){
    ...
    drawer.classList.add('open');overlay.classList.add('open');document.querySelector('#drawerContent').innerHTML=`<div class="match-detail-head"><p class="eyebrow">MATCH INTELLIGENCE</p><h2>${display(home)}<br><span class="versus">VS</span><br>${display(away)}</h2><span>比赛数据中心 · ${window.uelSeason.label} 欧罗巴</span></div><div class="loading-detail"><div class="loading-ball"></div>正在整理历史交锋与赛季数据…</div>`;
    ...
    if(request!==detailRequest||!drawer.classList.contains('open'))return;
    const homeGames=matches.filter(m=>m[1]===home||m[2]===home),awayGames=matches.filter(m=>m[1]===away||m[2]===away);
    document.querySelector('#drawerContent').innerHTML=`<div class="match-detail-head"><p class="eyebrow">MATCH INTELLIGENCE</p><h2>${display(home)}<br><span class="versus">VS</span><br>${display(away)}</h2><span>${date?`${window.uelSchedule?window.uelSchedule.detailLabel(date,time):date+' '+time} · `:''}比赛数据中心 · ${window.uelSeason.label} 欧罗巴</span></div><div id="predictionEditor"></div><div class="detail-tabs"><button class="active" data-jump="h2h">历史交锋</button><button data-jump="seasonForm">赛季战绩</button><button data-jump="futureGames">${window.uelSeason.current?'未来赛程':'联赛赛程'}</button></div>${failed?'<div class="detail-error">历史数据源连接失败，当前仅展示本赛季本地数据。</div>':''}<section class="detail-section" id="h2h"><h3>历史交手记录</h3><p>${window.uelSeason.current?'近期最多 12 次欧罗巴交锋':'所选赛季归档内双方交锋'}</p><div class="detail-list">${tableHead()}${h2h.length?h2h.map(m=>scoreRow(m)).join(''):'<div class="detail-row"><time>—</time><span class="clubs">暂无可用交锋记录</span><span>—</span><strong>—</strong></div>'}</div></section><section class="detail-section" id="seasonForm"><h3>本赛季历史战绩</h3><p>两队${window.uelSeason.label}欧罗巴全部已完赛比赛</p><h4 class="team-divider">${display(home)} · ${homeGames.length} 场</h4><div class="detail-list">${tableHead()}${homeGames.map(m=>scoreRow(m)).join('')}</div><h4 class="team-divider">${display(away)} · ${awayGames.length} 场</h4><div class="detail-list">${tableHead()}${awayGames.map(m=>scoreRow(m)).join('')}</div></section><section class="detail-section" id="futureGames"><h3>${window.uelSeason.current?'本赛季未来比赛':'本赛季联赛阶段赛程'}</h3><p>${window.uelSeason.current?'依据当前已公布的欧罗巴赛程':'UEFA官方144场联赛阶段归档'}</p><h4 class="team-divider">${display(home)}</h4><div class="detail-list">${futureRows(home)}</div><h4 class="team-divider">${display(away)}</h4><div class="detail-list">${futureRows(away)}</div></section><div class="data-source-panel"><b>比赛详情数据来源</b><span>${window.uelSeason.current?'历史交锋、本赛季战绩和未来赛程':'UEFA官方比赛与积分榜静态归档'}</span>${window.uelSeason.current?'<a href="https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.europa/scoreboard" target="_blank" rel="noopener">ESPN 历史赛果</a>':''}<a href="https://www.uefa.com/uefaeuropaleague/fixtures-results/" target="_blank" rel="noopener">UEFA 赛程与赛果</a></div>`;
    if(window.uelSeason.current)mountPredictionEditor(document.querySelector('#predictionEditor'),predictionKey,home,away,predictions[predictionKey]);else document.querySelector('#predictionEditor').innerHTML='<div class="detail-error">历史归档为只读模式，不提供赛前预测。</div>';
    ...
    document.querySelectorAll('.detail-tabs button').forEach(b=>b.onclick=()=>{document.querySelectorAll('.detail-tabs button').forEach(x=>x.classList.remove('active'));b.classList.add('active');const section=document.querySelector('#'+b.dataset.jump);drawer.scrollTo({top:section.getBoundingClientRect().top-drawer.getBoundingClientRect().top+drawer.scrollTop-64,behavior:'smooth'})});
  }
  ...
  document.querySelector('#updateBtn').onclick=()=>window.uelSeason.current?updateData(false):location.reload();
  ...
  if(window.uelSeason.current){
    ...
    window.renderUelAdvancement?.();
    window.uelDashboardUpdate?.({archive:true,checkedAt:window.uelArchive?.source?.generatedAt,source:'UEFA 官方静态赛季归档',coverage:{league:true,qualifying:true,knockout:true}},false);
  }
})(); 
