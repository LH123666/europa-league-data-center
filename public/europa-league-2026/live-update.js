(function(){
  ...
  const predictionStorageKey='uel36-match-predictions-v1';
  ...
  const teamNoteStorageKey='uel36-team-notes-v1';
  ...
  const renderSchedule=()=>{
    ...
    count.textContent=`${upcoming.length} 场待赛 · 欧洲中部时间`;
    ...
    grid.innerHTML=Object.entries(days).slice(0,9).map(([date,fs])=>`<article class="fixture-day"><div class="fixture-date"><span>${date.slice(5).replace('-','月')}日</span><b>${['日','一','二','三','四','五','六'][new Date(date+'T12:00:00').getDay()]}</b></div>${fs.map(f=>`<div class="fixture" data-date="${f.date}" data-time="${f.time}" data-home="${encodeURIComponent(f.home)}" data-away="${encodeURIComponent(f.away)}"><time>${f.time} · CET/CEST · 欧罗巴</time><div class="fixture-teams"><span>${display(f.home)}</span><i>VS</i><span>${display(f.away)}</span></div>${predictionSummary(f)}<small class="fixture-more">查看比赛详情 →</small></div>`).join('')}</article>`).join('');
  ...
  async function openFixtureDetail(home,away,date='',time=''){
    ...
    drawer.classList.add('open');overlay.classList.add('open');document.querySelector('#drawerContent').innerHTML=`<div class="match-detail-head"><p class="eyebrow">MATCH INTELLIGENCE</p><h2>${display(home)}<br><span class="versus">VS</span><br>${display(away)}</h2><span>比赛数据中心 · 2026/27 欧罗巴</span></div><div class="loading-detail"><div class="loading-ball"></div>正在整理历史交锋与赛季数据…</div>`;
    ...
    document.querySelector('#drawerContent').innerHTML=`<div class="match-detail-head"><p class="eyebrow">MATCH INTELLIGENCE</p><h2>${display(home)}<br><span class="versus">VS</span><br>${display(away)}</h2><span>${date?`${date}${time?` · ${time}`:''} · `:''}比赛数据中心 · 2026/27 欧罗巴</span></div><div id="predictionEditor"></div><div class="detail-tabs"><button class="active" data-jump="h2h">历史交锋</button><button data-jump="seasonForm">赛季战绩</button><button data-jump="futureGames">未来赛程</button></div>${failed?'<div class="detail-error">历史数据源连接失败，当前仅展示本赛季本地数据。</div>':''}<section class="detail-section" id="h2h"><h3>历史交手记录</h3><p>近期最多 12 次欧罗巴交锋</p><div class="detail-list">${tableHead()}${h2h.length?h2h.map(m=>scoreRow(m)).join(''):'<div class="detail-row"><time>—</time><span class="clubs">暂无可用交锋记录</span><span>—</span><strong>—</strong></div>'}</div></section><section class="detail-section" id="seasonForm"><h3>本赛季历史战绩</h3><p>两队2026/27欧罗巴全部已完赛比赛</p><h4 class="team-divider">${display(home)} · ${homeGames.length} 场</h4><div class="detail-list">${tableHead()}${homeGames.map(m=>scoreRow(m)).join('')}</div><h4 class="team-divider">${display(away)} · ${awayGames.length} 场</h4><div class="detail-list">${tableHead()}${awayGames.map(m=>scoreRow(m)).join('')}</div></section><section class="detail-section" id="futureGames"><h3>本赛季未来比赛</h3><p>依据当前已公布的欧罗巴赛程</p><h4 class="team-divider">${display(home)}</h4><div class="detail-list">${futureRows(home)}</div><h4 class="team-divider">${display(away)}</h4><div class="detail-list">${futureRows(away)}</div></section><div class="data-source-panel"><b>比赛详情数据来源</b><span>历史交锋、本赛季战绩和未来赛程</span><a href="https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.europa/scoreboard" target="_blank" rel="noopener">https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.europa/scoreboard</a><a href="https://www.uefa.com/uefaeuropaleague/fixtures-results/" target="_blank" rel="noopener">https://www.uefa.com/uefaeuropaleague/fixtures-results/</a></div>`;
